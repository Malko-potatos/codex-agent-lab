...
- 설명의 편의를 위해 반복해서 나오는 용어는 `docs/plain-language-glossary.md`에 고정해 둡니다.
...
다만 같은 용어를 문서마다 다르게 설명하면 헷갈릴 수 있습니다. 그래서 반복해서 나오는 용어는 `docs/plain-language-glossary.md`에 명시해 두고, 다른 문서에서는 그 뜻을 기준으로 사용합니다.

## 학습 문서

- [쉬운 용어 사전](docs/plain-language-glossary.md): 반복해서 나오는 용어를 수강생이 이해하기 쉬운 말로 정리합니다.
- [아이디어 검토 체크리스트](docs/idea-review-checklist.md): 새 에이전트 아이디어를 실제로 만들기 전에 반복성, 입력 데이터, 산출물, 외부 연동, 리스크를 확인합니다.
- [보안과 개인정보 원칙](docs/security-and-privacy.md): 공개 저장소에 넣으면 안 되는 자료와 외부 작업 승인 기준을 확인합니다.
- [Codex 메모리 정책](docs/codex-memory-policy.md): Codex memory를 언제 쓰고, 무엇을 저장하지 말아야 하는지 확인합니다.
- [하네스 번역 skill](agents/00-agent-harness-translator/skills/translate-agent-harness/SKILL.md): 0번 에이전트가 업무 시나리오를 Codex 하네스 명세로 번역하는 절차를 확인합니다.
- [Skill, 도구, MCP 선택 카탈로그](agents/00-agent-harness-translator/skills/translate-agent-harness/references/skill-tool-mcp-catalog.md): 0번 에이전트가 수강생에게 추천할 수 있는 skill, 도구, 외부 서비스 후보를 확인합니다.
- [Workflow 상태 검증 가이드](agents/00-agent-harness-translator/skills/translate-agent-harness/references/workflow-state-validation.md): 0번 에이전트가 시나리오의 부족 정보와 보강할 하네스 부품을 판단하는 기준을 확인합니다.
- [첫 주차 에이전트 선발 기록](docs/week-1-agent-selection.md): 네 개의 아이디어 대화에서 첫 주차에 만들 에이전트를 고른 기준과 결과를 확인합니다.
...
`-- agents/
    ...
    |           |   `-- openai.yaml
    |           |-- assets/
    |           |   |-- harness-analysis-view.html
    |           |   `-- harness-parts-spec.md
    |           `-- references/
    |               |-- build-checklists.md
    |               |-- harness-part-decision-tree.md
    |               |-- memory-policy.md
    |               |-- plain-language-glossary.md
    |               |-- skill-tool-mcp-catalog.md
    |               `-- workflow-state-validation.md
...
2. 각 아이디어를 `agents/00-agent-harness-translator/skills/translate-agent-harness/SKILL.md` 기준으로 번역합니다.
