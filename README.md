...
1. 모든 에이전트는 기본적으로 `draft-first`, `human-review-required` 방식으로 설계합니다.
2. 외부 전송, 업로드, 메시지 발송, 크롤링, 로그인 세션 사용은 명시 승인 후 실행합니다.
3. 공개 저장소에는 샘플 데이터만 둡니다.
4. 실제 자동화 구현 전에는 `docs/idea-review-checklist.md`로 사업성, 법적 리스크, 데이터 리스크를 검토합니다.
5. 에이전트가 만든 산출물은 초안이며 최종 제출물은 사람이 검토합니다.

## 다음 단계

1. 우선순위가 높은 에이전트 2-3개를 고릅니다.
2. 각 에이전트 폴더에 샘플 입력과 기대 출력 예시를 추가합니다.
3. 필요한 외부 서비스와 승인 흐름을 정합니다.
4. 실제 자동화 스크립트, 템플릿, 테스트를 추가합니다.
