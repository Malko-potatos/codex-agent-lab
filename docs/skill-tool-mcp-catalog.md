...
- 처음부터 많은 도구를 고르지 않습니다. 첫 주차에는 local draft, mock data, 사람 검토로 시작합니다.
...
| 문서/스프레드시트/프레젠테이션 도구 | `.docx`, `.xlsx`, `.pptx` 같은 업무 산출물을 다룹니다. | 첫 주차 이후 변환 후보로 검토 | 첫 주차에는 markdown/json 초안부터 시작합니다. |
...
| 첫 주차 사용법 | 실제 `.hwp` 자동 생성은 하지 않고, “나중에 HWP 확인/내보내기 후보”로만 기록합니다. |
...
아래 후보는 실제 계정, 로그인, API 권한이 필요할 수 있습니다. 첫 주차에는 대부분 mock data 또는 dry-run으로만 다룹니다.
...
| 네이버웍스 | 메일, 캘린더, 메시지, 파일을 다룹니다. | 첫 주차에는 후보로만 기록 | 로그인, 메시지, 파일 접근 |
...
Google Drive 연동은 실제 회사 문서를 읽어야 하므로 첫 주차에는 제외합니다.
